...
: **완전이진트리의 일종**으로, 데이터에서 **최대/최소값을 빠르게 찾아내도록** 만들어진 자료구조

: 큰 키(우선 순위)에 자주 액세스하거나 이를 중심으로 정렬된 시퀀스를 활용할 때 유용하다.

: 중복된 값을 허용한다.(BST(Binary Search Tree. 이진 탐색 트리))에서는 중복된 값을 허용하지 않는다.
...
- Best, Average, Worst Case
  ...
  이 과정을 거치는 요소가 모두 $n$개 이므로 **시간복잡도는 Best, Average, Worst Case 모두 $O(n\log_2n)$다.**
