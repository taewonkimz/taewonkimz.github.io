...
기존에 $p$(시작점)부터 $r$(끝점)까지 데이터가 있었다면 , $q$(기준점)를 기준으로 데이터를 두 개로 쪼갠 뒤, 다시 쪼개진 각각의 데이터의 $p$와 $r$로부터 $q$를 선정하여 쪼개면 이번에는 데이터가 4개로 분리된다.

이 과정을 계속...................계속...............쪼갤 수 없을때 까지 반복하면 데이터가 최소의 단위로 분리가 되고,

이렇게 쪼개진 데이터를 두 개씩 비교하며 정렬하고 합친다. 그다음, 다시 합쳐진 두 데이터끼리 비교해서 정렬하고 합친다. 이 과정을 계속..... 반복하면 최종적으로 데이터가 모두 정렬되어 합쳐지는 것이다.
